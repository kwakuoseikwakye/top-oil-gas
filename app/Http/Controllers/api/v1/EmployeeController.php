...
class EmployeeController extends Controller
{
    public function index()
    {
        return response()->json([
            "data" => DB::table("tblstaff")->where("deleted", 0)
    ...
    {
        return response()->json([
            "data" => DB::table("tblstaff")->where("deleted", 1)
                ->orderByDesc("createdate")->get(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            ModelsLog::insert([
                ...
                "module" => "Employee",
                "action" => "Add",
                "activity" => "Employee added from mobile with id ST-{$transid} successfully",
                "ipaddress" => $userIp,
                "createuser" =>  $request->createuser,
    ...
    public function update(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                "staffID.required" => "No staff ID supplied",
                "firstName.required" => "No first name supplied",
                "lastName.required" => "No last name supplied",
            ...
            DB::beginTransaction();
            $transid = strtoupper(bin2hex(random_bytes(4)));
            DB::table("tbluser")->where("userid", $request->staffID)->update([
                "fname" => $request->firstName,
                "lname" => $request->lastName,
                "phone" => empty($request->phoneNumber) ? '' : $request->phoneNumber,
                "email" => empty($request->email) ? '' : $request->email,
            ...
            DB::table('tblstaff')->where("staffid", $request->staffID)->update([
                "fname" => strtoupper($request->firstName),
                "lname" => strtoupper($request->lastName),
                "mname" => strtoupper($request->middleName),
                "region" => $request->region,
            ...
            ModelsLog::insert([
                ...
                "module" => "Employee",
                "action" => "Update",
                "activity" => "Employee updated from mobile with id ST-{$transid} successfully",
                "ipaddress" => $userIp,
                "createuser" =>  $request->createuser,
                "createdate" => gmdate("Y-m-d H:i:s"),
                "longitude" => $locationData->longitude ?? $userIp,
    ...
    public function delete(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            "staffID.required" => "No employee id supplied",
        ...
        try {
            DB::beginTransaction();
            $user = User::where("userid", $request->staffID)->where("deleted", 0)->first();
            ...
            $customer = DB::table("tblstaff")->where("staffid", $request->staffID)
            ...
            DB::rollBack();
            Log::error("An error occured deleting employee", [
                "errMsg" => $e->getMessage(),
                "trace" => $e->getTrace(),
            ]);
